...
class WinnerChecker {
    ...
    WinnerChecker(PlayerSymbol[][] board) {
        this.board = board;
    }

    boolean isWinner(PlayerSymbol symbol) {
    ...
    boolean isRowFilled(int row, PlayerSymbol symbol) {
        return isLineFilled(row, 0, 0, 1, symbol);
    }

    boolean isColFilled(int col, PlayerSymbol symbol) {
        return isLineFilled(0, 1, col, 0, symbol);
    }

    boolean isLineFilled(int rowBegin, int rowStep, int colBegin, int colStep, PlayerSymbol symbol) {
